...
class DetectionEngine:
    ...
    def detect_deadlock(self):
        try:
            ...
            self.logger.log(f"Cycle detection result: {cycle_result.get('has_deadlock', False)}")

            bankers = BankersAlgorithm(self.system_state)
            bankers_result = bankers.detect_deadlock()

            self.logger.log(f"Banker's algorithm result: {bankers_result.get('has_deadlock', False)}")

            # ── Consistency rule ──────────────────────────────────────────────
            # If a cycle is detected the system MUST be considered unsafe.
            # Override Banker's result to stay consistent.
            if cycle_result.get('has_deadlock') and bankers_result.get('is_safe'):
            ...
            has_deadlock = cycle_result.get('has_deadlock', False) or bankers_result.get('has_deadlock', False)
            ...
            self.logger.log_error(f"Error in detect_deadlock: {e}")
            # Return safe defaults
            empty_graph = nx.DiGraph()
            return {
                'deadlock': False,
                'involved_processes': [],
                'cycle_detection': {'has_deadlock': False, 'cycles': [], 'involved_processes': []},
                'bankers_algorithm': {'has_deadlock': False, 'is_safe': True, 'state': 'safe'},
    ...
    def visualize_graph(self, graph, output_path=None):
        try:
            ...
            process_nodes  = [n for n, d in graph.nodes(data=True) if isinstance(d, dict) and d.get('node_type') == 'process']
            resource_nodes = [n for n, d in graph.nodes(data=True) if isinstance(d, dict) and d.get('node_type') == 'resource']
            request_edges    = [(u, v) for u, v, d in graph.edges(data=True) if isinstance(d, dict) and d.get('edge_type') == 'request']
            allocation_edges = [(u, v) for u, v, d in graph.edges(data=True) if isinstance(d, dict) and d.get('edge_type') == 'allocation']
            ...
            self.logger.log_error(f"Error in visualize_graph: {e}")
